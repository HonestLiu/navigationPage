...
// /data，绝不会用挂载把 /app 下的代码（含 server/index.js）遮掉。
...
// 模块级可变引用：路由内务必用 getDb() 取最新引用，避免 reset 后拿到旧对象
function getDb() { return db; }
function setDb(v) { db = v; }
function resetDb() { db = defaultDB(); saveDB(db); return db; }

module.exports = {
    DATA_DIR, DB_PATH, UPLOAD_DIR, WALLPAPER_DIR,
    getDb, setDb, saveDB, resetDb
};
